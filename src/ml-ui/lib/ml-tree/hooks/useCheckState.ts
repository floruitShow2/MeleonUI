...
export default function useCheckedState(props: CheckedStateInput) {
    ...
    watchEffect(() => {
        if (propCheckedKeys?.value) {
            [computedCheckedKeys.value, computedIndeterminateKeys.value] = getDefaultStateWithKeys(localCheckedKeys.value)
    ...
    return {
        ...
        setCheckedState(newCheckedKeys: string[], newIndeterminateKeys: string[], reinitialize = false) {
            if (reinitialize) {
                init(newCheckedKeys)
            } else {
